...
Class SCIahk
{
	...
	/**
	 * turn off case sense (for AHK), do this before setting keywords
	 * @param {Integer} bolean 
	 * @returns {Integer} 
	 */
	SetCase(bolean:=false) =>	this.Ctrl.CaseSense := bolean 
	/**
	 * modify braces list that will be trackedmodify braces list that will be tracked
	 * @param {String} Brace 
	 * @returns {String} 
	 */
	SetBrace(Brace:="[]{}()") => this.Ctrl.Brace.Chars := Brace ; 

	/**
	 * set this to "\" to load up CustomLexer.c, or to "``" to load an 	
	 * @param {String} char 
	 * @returns {String} 
	 */
	SetEscChar(char:="``")    => this.Ctrl.SyntaxEscapeChar := char 

	/**
	 * 	SetCommentChar(char:=";") => this.Ctrl.SyntaxCommentLine := ";"
	 * @param {String} char 
	 * @returns {String} 
	 */
	SetCommentChar(char:=";") => this.Ctrl.SyntaxCommentLine := ";"
	 ...
	 * @returns {Integer} 
	 */
	SetTab(bolean:=false)     => this.Ctrl.Tab.Use := bolean
	 ...
	 * @returns {Integer} 
	 */
	SetTabWidth(w:=4)         => this.Ctrl.Tab.Width := 4
	/**
	 * move to the beginning of the document
	 * @param {Integer} pos 
	 * @returns {Integer} 
	 */
	SetCurPos(pos:=0)		  => this.Ctrl.CurPos := pos 

	/**
	 * this doesn't do anything for now
	 * @param callback 
	 */
	SetCalback(callback)      => this.Ctrl.callback := callback 
	/**
	 * turns syntax highlighting on
	 * @param {bolean} bolean 
	 * @returns {bolean} 
	 */
	SetHighlighting(bolean:=true) => this.Ctrl.CustomSyntaxHighlighting := bolean 
	...
	/**
	 * Default := "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_#"
	 * @param {String} char 
	 * @returns {String} 
	 */
