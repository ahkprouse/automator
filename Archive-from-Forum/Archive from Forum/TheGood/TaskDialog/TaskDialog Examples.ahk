#Include B:\Progs\AutoHotkey_L\Archive from Forum\TheGood\TaskDialog\TaskDialog.ahk
/*! TheGood
    TaskDialog() Examples
...
btnShow5Callback(hwnd, uNotification, wParam, lParam, dwRefData) {
    ...
    If (uNotification = 3) {
        ...
        ;Use the WidetoANSI() function to retrieve the address clicked
        TaskDialog_WidetoANSI(lParam, sLink)
        Run, %sLink%
    }
}
...
btnShow15Callback(hwnd, uNotification, wParam, lParam, dwRefData) {
    ...
    Else If (uNotification = 2) {
        ...
        If (wParam = 1001) { ;Change Icon
            ...
            TaskDialog_ANSItoWide(&s, sW)

            ;Update footer using a TDM_UPDATE_ELEMENT_TEXT message
            SendMessage, 0x400 + 114, 2, &sW,, ahk_id %hwnd%
...
btnShow16Callback(hwnd, uNotification, wParam, lParam, dwRefData) {
    ...
    If (uNotification = 4) {
        ...
        TaskDialog_ANSItoWide(&sElapsed, sElapsedW)

        ;Send a TDM_UPDATE_ELEMENT_TEXT message
        SendMessage, 0x400 + 114, 2, &sElapsedW,, ahk_id %hwnd%
