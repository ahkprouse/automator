...
NotifyV2 := LoadNotifyLib()

Path := FileSelect('D',,'Where would you like to Save the Library')
if !DirExist(Path)
	return
FileAppend(NotifyV2,path "\NotifyV2.ahk",'utf-8')
...
Class MultiGui
{
	...
	__new(info)
	{
		...
		if info.GenMonitor > 1
			taskbarDock := "B"
		else
			taskbarDock := MultiGui.Taskbar.Docked
		if MultiGui.Guis.Length > 1 ;IsSet(LastNote)
		{
			Switch taskbarDock
			{
				...
				if( (MultiGui.LastPOs.y - h) > 0)
					POS := "x" MultiGui.Monitors[info.GenMonitor]['w'] - w " y" MultiGui.LastPOs.y - h -1 
				else
					POS := "x" MultiGui.LastPOs.x - w " y" MultiGui.Monitors[info.GenMonitor]['h'] - MultiGui.TaskBar.h - h 
				Case "R":
				if( (MultiGui.LastPOs.y - h) > 0)
					POS := "x" MultiGui.LastPOs.x " y" MultiGui.LastPOs.y - h - 1
				else
					POS := "x" MultiGui.LastPOs.x - w " y" MultiGui.Monitors[info.GenMonitor]['h'] - h
			}
		}
		else
		{
			...
			{
				Case "B":
				POS := "x" MultiGui.Monitors[info.GenMonitor]['w'] - w " y" MultiGui.Monitors[info.GenMonitor]['h'] - MultiGui.TaskBar.h - h
				Case "R":
				POS := "x" MultiGui.Monitors[info.GenMonitor]['w'] - w - MultiGui.TaskBar.w " y" MultiGui.Monitors[info.GenMonitor]['h'] - h
				default:
				POS := "x" MultiGui.Monitors[info.GenMonitor]['w'] - w " y" MultiGui.Monitors[info.GenMonitor]['h'] - h
			}
		}
