...
Class triggers
{
	...
	static __New()
	{
		...
		this.LoadAllHotkeys()
		this.ui := Gui()
		this.ui.OnEvent('Close', (*)=>this.ui.Hide())
	...
	static addCheckbox(Callback,Label,list:=Map(),line:= 3)
	{
		...
		for ChkName, chkVal in list
		{
			...
			this.Actions[name][chk]     := this.ui.Addcheckbox(opt chked 'checked'  ,ChkName)
		}
	}
	...
	static Show(opt:='') => (this.UI.show(opt),StartPrefMicBarUpdate())
	...
	static FinishMenu(name:='Preferences',setDefault:=false)
	{
		...
		if this.finished
			return
		this.tray.add()
		this.tray.add(name,(*)=>(this.UI.Show(),StartPrefMicBarUpdate()))
		this.tray.add()
		...
		this.UI.AddButton('x+m w80','Cancel').onEvent('click',(*)=>this.closetriggers())
		if	setDefault
		{
			this.tray.Default := name	
			this.tray.ClickCount := 1
		}
		this.finished := true
	}
