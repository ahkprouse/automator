...
Win_Recall(Options, Hwnd="", IniFileName=""){
	...
	if (op = "<") {
		...
		else ;RegRead, pos, REG_SZ,   HKEY_CURRENT_USER, %key%, %A_ScriptFullPath%!%name%  Removed by Maestrith as this was depricated approach
			RegRead, pos,  HKEY_CURRENT_USER, %key%, %A_ScriptFullPath%!%name% ;Maestrith updated to use newer settings
		if (pos = "") or !Hwnd 
			return pos
...
/*
	...
	o Licensed under Unlicense. For more info visit <http://unlicense.org/>
	/*
