 ...
 * @Library  : Triggers                                                         *
 * @Verson   : v 0.2.3                                                          *
 * @DevPath  : S:\lib\v2\Preferences\Triggers.ahk                               *
 * @Author   : Xeo786                                                           *
...
Class triggers
{
	...
	; ***************** add checkbox **********************
	static addCheckbox(Callback,Label,list:=Map(),line:= 3)
	{
		if this.finished
		...
		for ChkName, chkVal in list
		{
			...
			if a_index = 1
			{
				...
				opt := 'xp+20 yp+30 ' chked 'checked'
			}
			else if Mod(a_index, 3) = 1
				opt := 'xm ' chked 'checked'
			else
				opt := 'x+m ' chked 'checked'
			
			this.Actions[name][ChkName]     := this.ui.Addcheckbox(opt chked 'checked'  ,ChkName)
		}
	}
	...
	Static Add(Callback,Label,DefaultHotkey,Title, type, trayAction)
	{
		...
		this.Actions[name]['HK']     	:= 	this.ui.AddEdit('x+m w440 h27 readonly -multi', text :=  this.HKToString(DefaultHotkey) (Title ?? ' - ' Title) )
		this.UI.SetFont('s12','Verdana')
		...
		if trayAction
			...
			this.tray.add(this.Actions[name]['tray'] := Label '`t' (hstr ? hstr DefaultHotkey:this.HKToString(DefaultHotkey)),(*)=>this.Show())
	...
	static FinishMenu(name:='Preferences',setDefault:=false)
	{
		...
		this.save.onEvent('click',(*)=>this.ApplyTriggers())
	...
	static disableTriggers()
	{
		for name, action in this.Actions
		{
			if !this.Actions[name].has('trigger')
				continue
			...
			if ddl = 'hotstring'
				Hotstring(hst := '::' this.Actions[name]['trigger'],callback,'Off')
			else
	...
	static BuildChildGuis()
	{
		...
		; Hotkeys Gui
		Crosshairbg := "compstui.dll" ; 'c:\Windows\Cursors\cross_rl.cur'
		this.CH := {h:50,w:50}

		this.HKUI := Gui('Owner' this.ui.hwnd,'Select Hotkey')

		this.HKUI.SetFont('s12','Verdana')
		this.HKCR := this.HKUI.addpic('y30  border icon52 h' this.CH.h ' w' this.CH.w,Crosshairbg)
					 this.HKUI.AddText('xp+70 yp-10','Context (WinTitle):')
					 this.HKUI.SetFont('s12','Arial Narrow')
		this.HKCX := this.HKUI.AddEdit('xm+70 y+m w300')
					 this.HKUI.SetFont('s12','Verdana')
		; this.HKUI.AddPic('x+m h27 w-1 +border')

		this.HKUI.AddText('xm y+40 right','Select Hotkeys:')
		this.HKWin   := 	this.HKUI.AddCheckbox('xm','Window')
		this.HKCtrl  := 	this.HKUI.AddCheckbox('x+m','Control')
		...
		this.MSCR :=  this.MSUI.addpic('icon52 h' this.CH.h ' w' this.CH.w,Crosshairbg)
					  this.MSUI.AddText('xp+70 yp+10','Context (WinTitle):')
					  this.MSUI.SetFont('s12','Arial Narrow')
		this.MSCX :=  this.MSUI.AddEdit('xm+70 y+m w300')
					  this.MSUI.SetFont('s12','Verdana')

		this.MSUI.AddText('xm y+40','Select Mouse Trigger:')
		this.MSWin   := 	this.MSUI.AddCheckbox('xm','Window')
		this.MSCtrl  := 	this.MSUI.AddCheckbox('x+m','Control')
		this.MSShift := 	this.MSUI.AddCheckbox('x+m','Shift')
		this.MSAlt   := 	this.MSUI.AddCheckbox('x+m','Alt')
		this.MS      :=   this.MSUI.AddDDL('xm w330',['Left','Right','Middle','Special Button 1','Special Button 2'])
		...
		this.HSCR :=  this.HSUI.addpic('icon52 h' this.CH.h ' w' this.CH.w,Crosshairbg)
					  this.HSUI.AddText('xp+70 yp+10','Context (WinTitle):')
					  this.HSUI.SetFont('s12','Arial Narrow')
		this.HSCX :=  this.HSUI.AddEdit('xm+70 y+m w300')
					  this.HSUI.SetFont('s12','Verdana')
	...
	static toggleModifiers(name)
	{
		if this.Actions[name].has('DisableModifiers')
		&& this.Actions[name]['DisableModifiers']
		{	
			x := this.MSWin
			this.MSWin.visible := false
			this.MSCtrl.visible := false
			this.MSShift.visible := false
			this.MSAlt.visible := false

			this.HKWin.visible := false
			this.HKCtrl.visible := false
			this.HKShift.visible := false
			this.HKAlt.visible := false
		}
		else
		{
			this.MSWin.visible := true
			this.MSCtrl.visible := true
			this.MSShift.visible := true
			this.MSAlt.visible := true

			this.HKWin.visible := true
			this.HKCtrl.visible := true
			this.HKShift.visible := true
			this.HKAlt.visible := true
		}
	}
