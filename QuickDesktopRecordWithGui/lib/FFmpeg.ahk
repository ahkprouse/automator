 ...
 * @author RaptorX
 * @version 1.3.0
 * @description This class provides methods to interact with FFmpeg for various media processing tasks.
 * - [Documentation](https://www.ffmpeg.org/documentation.html)
 *
 * ---
 * #### Properties
 * @prop {String}  path The path to the ffmpeg executable.
 *
 * ---
 * #### Methods
 * @method VideoToAudio Converts a video file to an audio file.
 * @method AddSubtitles Adds subtitles to a video file.
 * @method CreateSections Splits a media file into sections.
 * @method Trim Trims a media file.
 * @method GetType Gets the type of a media file.
 * @method GetVersion Gets the version of FFmpeg.
 * @method Getinfo Gets information about a media file.
 * ----
 * #### Child Classes
 * @class None
 *
 * ---
 * #### Notes
 * This class requires FFmpeg to be installed and accessible in the system's PATH.
 */
class FFMPEG {
	static pid  := 0
	static hwnd := 0
	...
	static path {
		...
		set {
			...
			if Value
			{
				SplitPath value, , &ffmpeg_dir
				Paths.Set(ffmpeg_dir, true)
			...
			{
				fixed_paths := ''
				SplitPath FFMPEG.path, , &ffmpeg_dir
				for path in Paths
				{
					if path = ffmpeg_dir
						continue
	...
	static GetVersion()
	{
		...
		if RegExMatch(outtxt, "ffmpeg version (\d+.\d+.\d+)", &match)
			return match[1]
		else
	...
	static GetDevices()
	{
		...
		loop
		{
			...
			if (devName != "")
				allDevices.Push(devName)
			
			; Enumerate components for this device
			cmpMap := Map()
			loop
			{
				try
				{
					...
					if (cmpName != "")
						allDevices.Push(Qualify(cmpName, cmpMap, A_Index))
				}
				catch
					break
			}
		...
		return allDevices
		; Qualifies names with ":index" when needed
	...
	static SetupRecording(mic, out_path)
	{
		static cmd :=
		(LTrim Join`s
		'ffmpeg
			...
			-f dshow
			-i audio="{}"
			-af acompressor=threshold=-20dB:ratio=4:attack=200:release=1000,highpass=f=200,lowpass=f=3000,afftdn=nf=-25,adeclick,adeclick,volume=5dB
			-acodec libopus
			-b:a 24k
			-ac 1
			-application voip
		...
		while pid := ProcessExist('ffmpeg.exe')
			ProcessClose(pid)

		Run Format(cmd, mic, out_path),, 'hide'
		ProcessWait('ffmpeg.exe')
		FFMPEG.hwnd := WinWait('ahk_exe ffmpeg.exe')
		FFMPEG.pid  := WinGetPID('ahk_exe ffmpeg.exe')
		FFMPEG.ControlProcess('Suspend')
	...
	static StartRecording() => FFMPEG.ControlProcess('Resume')
	static PauseRecording() => FFMPEG.ControlProcess('Suspend')
	static StopRecording()  => FFMPEG.ControlProcess('Terminate')

	/**
	 *
	 * @param {'Terminate'|'Suspend'|'Resume'} action
	 * @returns {Number}
	 */
	Static ControlProcess(action)
	{
		...
		{
			try {
				FFMPEG.pid := ProcessExist('ffmpeg.exe') || WinGetPID('ahk_exe ffmpeg.exe')
			} catch {
				FFMPEG.pid := 0
				return
			}
		...
		If !h:=DllCall("OpenProcess", "uInt", 0x1F0FFF, "Int", 0, "Int", FFMPEG.pid)
			throw Error('Coulnd not open process', A_ThisFunc, FFMPEG.pid)
		
		switch action
		{
		case 'Terminate':
			WinClose(FFMPEG.hwnd)
			while ProcessExist(FFMPEG.pid) ; give it time to properly save audio
			{
				; WinClose(FFMPEG.hwnd)
				ProcessWaitClose(FFMPEG.pid,5)
			}
			res := 0
			FFMPEG.pid := 0
			FFMPEG.hwnd := 0
		case 'Suspend':
			res := DllCall("ntdll.dll\NtSuspendProcess", "Int", h)
		case 'Resume':
			res := DllCall("ntdll.dll\NtResumeProcess", "Int", h)
		}
