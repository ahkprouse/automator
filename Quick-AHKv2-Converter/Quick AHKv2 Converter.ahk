...
;******************************************************************************
; heavily borrowed from Mikeww's repo https://github.com/mmikeww/AHK-v2-script-converter
#SingleInstance
...
#Include <2Functions>
#Include <3Methods>
#Include <4ArrayMethods>
#Include <5Keywords>

if !A_IsCompiled
   TraySetIcon 'res\main.ico'

Editor := Gui('+Resize','Quick Edit')
Editor.OnEvent('Size', ResizeWindow)
Editor.SetFont('s16','Courier New')
Editor.AddEdit('vConvertedText w600 r20')
editor.SetFont()
Editor.AddButton('vCopybtn x' 600-75+Editor.MarginX ' w75', 'Copy').OnEvent('Click', (*)=>(Editor.Submit(), A_Clipboard := Editor['ConvertedText'].value))
Editor.AddStatusBar()

; Switch this hotkey to your preferred one
^+c::
{
   A_Clipboard := ""
   ...
   if !A_Clipboard
      return MsgBox('Could not copy selected text', 'Error', 'IconX')

   Editor['ConvertedText'].Value := Convert(A_Clipboard)
   Editor.Show()
}

ResizeWindow(edt, MinMax, Width, Height)
{
   static prevRect := ''
   ; edt.GetClientPos(&edtX, &edtY, &edtW, &edtH)

   if !Width && !Height
      return

   ; when running fo the first time set the previous rectangle to
   ; the current values so that the differenc is zero
   if !prevRect
      prevRect := {w:Width, h:Height}

   diff := {w : Width - prevRect.w, h : Height - prevRect.h}
   ...
   for ctrl in edt
   {
      ...
      switch ctrl.type {
         case 'Edit':
            ctrl.Move(,,ctrlW + diff.w, ctrlH + diff.h)
         case 'Button':
            ctrl.Move(ctrlX + diff.w, ctrlY + diff.h)
            ctrl.Redraw()
      }
   }
}
