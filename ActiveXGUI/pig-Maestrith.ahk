...
imagepath:="B:\Progs\AutoHotkey_L\AHK Work\AHK_Functionality\GUIs\ActiveX\test.bmp"
...
/*
	img:=oPig[100].img
	img.ParentNode.RemoveChild(img) ;remove an image object
	oPig.Delete(100)
*/
...
Move_Pigs()
{
	...
	/*
		if(A_TickCount-tick>max)
			max:=A_TickCount-tick
		t(A_TickCount-tick,"Max: " max)
	*/
...
Place_New_Pig_On_Map()
{
	...
	*/
	wb.Body.Style.BackgroundColor:="#ff0000"
	img:=wb.CreateElement("img")
	oPig[Pig_Slot].img:=img
	img.src:=Pig_Png[oPig[Pig_Slot].3]
	Style:=img.Style
	for a,b in {left:XPig,top:YPig,position:"absolute",width:10,height:10}
		Style[a]:=b
	wb.Body.AppendChild(oPig[Pig_Slot].img)
}
Create_Arena() ;GUI 1
{
	global
	...
	/*
		Gui,1:Color,Gray
	*/
	Gui,Add,ActiveX,x300 y0 w1000 h700 vwb,mshtml:
...
Add_Control_Panel() ;GUI 2
{
	...
	Gui,2:Add,Button,xs w200 h40 Section gAdd_100_Pigs,Add 1000 New Pigs
	...
	Gui,2:Add,Picture,xs w200 h200  ,%imagepath%
...
Add_Background_Layer() ;GUI 3
{
	...
	Gui,3:Add,Picture,x0 y0 w1000 h10,%imagepath%
	Gui,3:Add,Picture,x0 y690 w1000 h10,%imagepath%
	Gui,3:Add,Picture,x0 y10 w10 h680,%imagepath%
	Gui,3:Add,Picture,x990 y10 w10 h680,%imagepath%
