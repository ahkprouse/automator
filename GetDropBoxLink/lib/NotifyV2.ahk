...
Class Notify
{
	Static Default :=
	{
		...
		GenDuration   : 7,
		GenSound	  : "tada",
		GenIcon		  : A_ScriptDir "\res\Dropbox.png",
		GenIconSize	  : 60,
	}
	...
	Static GenIconList()
	{
		...
		Notify.Show({
			...
			GenIcon:96,
			GenIconSize:50,
			Link:'Click this Link for <a href="https://help4windows.starlink.us/windows_8_shell32_dll.shtml">GenIcon List</a>`n' GenIconHelp
		})
	}
}
...
Class MultiGui
{
	...
	__new(info)
	{
		...
		if (Type(Info.GenIcon) = "Integer")
			MyGui.AddPicture("w" Info.GenIconSize " h" Info.GenIconSize " Icon" Info.GenIcon, MultiGui.ShellDll)
