...
Class triggers
{
	...
	; ***************** add checkbox **********************
	static addCheckbox(Callback,opt:='',label:='')
	{
		if this.finished
			throw Error('Cannot add new triggers after calling triggers.FinishMenu()')
		name := Callback.name
		chked := '-'
		if chk := IniRead(this.ini, 'CheckBoxs',label,0)
			chked := '+'
		else if !FileExist(this.ini)
		&& RegExMatch(opt,'(.*)?checked',&out)
			chked := out[1]
		else
			chked := '-'
		opt := RegExReplace(opt,'(.)?checked')
		chbx := this.ui.Addcheckbox(opt chked 'checked',label)
		chbx.onEvent('click',Callback)
		this.CheckBoxs[name] := Map('label',label,'callback',Callback,'ctrl',chbx)
		return chbx
		; h := 30
		...
		; 	&& label
		; 	{
		; 		this.CheckBoxs[name] := Map('label',Label,'callback',Callback)
		; 		this.CheckBoxs[name]['group'] 	:= 	this.ui.AddGroupBox('xm w600 h' h + 30, Label)
		; 		opt := 'xp+20 yp+30 section ' chked 'checked'
		; 		this.CheckBoxs[name][chk]     := this.ui.Addcheckbox(opt chked 'checked'  ,ChkName)
		; 		continue
		; 	}
		; 	else if Mod(a_index, line) = 1
	...
	static addRadios(Callback,list:=[])
	{
	...
	static FinishMenu(name:='Preferences',setDefault:=false)
	{
		...
		this.UI.AddButton('x+m w80','Cancel').onEvent('click',(*)=>this.hide())
		if	setDefault
	...
	static ApplyTriggers()
	{
		...
		for name, cb in this.CheckBoxs
		{
			IniWrite(cb['ctrl'].value, this.ini, 'CheckBoxs', cb['label'])
		}
	...
	{
		for name, action in this.Actions
			this.Hotkeyenable(action['callback'])
	}

	static Hotkeyenable(callback)
	{
		Name := callback.name
		...
		HotIfWinActive Title
		try 
			Hotkey(currenthotkey,callback,'on')
		catch
			try 
				Hotstring('::' currenthotkey,,'on')
	...
	{
		for name, action in this.Actions
			this.HotkeyRemove(action['callback'])
	}
