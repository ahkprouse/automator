...
SelectRange() {
	...
	InputBox, sid, Select Range, Select range:, , 600, 180, , , , , %gSelected%
	SetTimer, UpdateRange, off	
	if ErrorLevel
		return

	GuiControl, Focus, eMask
	SendInput, {RAW}%gSelRange%
}
...
;-------------------------------------------------------------------------------------
OnComboXButton:
	ComboX_Show( A_GuiControl="btnMask" ? hcxMask : hcxSR,,,600,600 )
return

;-------------------------------------------------------------------------------------
OnHLink() {
	run "https://the-Automator.com/Discover"
...
GuiDropFiles: 
	...
	if InStr(FileExist(gFiles), "D") && GetKeyState("CTRL")
	{
		folder := gFiles "\*.*", gFiles := ""
		loop, %folder% 
			gFiles .= A_LoopFileFullPath "`n"
	}
