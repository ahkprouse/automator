...
Ini_LoadSection( pIniFile, pSection="", pPrefix="inis_") {
 ...
 ;the fastest way to get single section
	if pSection !=
	{
		...
		Loop, % s-1								
			if !NumGet(res, (A_Index-1)*2, "UShort")			; Each line within the section is terminated with a null character.  Replace each delimiting null char with a newline
				NumPut(10, res, (A_Index-1)*2, "UShort")		; \0 -> \n
