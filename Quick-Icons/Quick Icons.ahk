...
;Changes: 
;Choose between Round/Square Icons
;Allow save to file
;****************************************************************************************************************************************************************************
...
Gui, Add, CheckBox, xm ym w90 Checked hwndhwnd gAdjustSlider , Round Icon
Main.RoundCheckBoxHwnd := hwnd

Gui, Add, CheckBox, xm y+m w90 Checked hwndhwnd gAdjustSlider , Add Border
...
Gui, Add, Button, xm w300 gSaveToFile , Save To File
...
SaveToFile:
	if( !Main.Change )
		gosub, AdjustSlider
	GuiControlGet, fName,, % Main.Edit3Hwnd
	FileSelectFile, fLocation, S24, % A_ScriptDir "\" fName ".png"
	...
	Gdip_SaveBitmapToFile(main.OutPBitmap, fLocation, 100)
return
...
ClipboardBase64:
	...
	fullVarName := "b64" Main.OutputVarName "_" Main.SliderValue "x" Main.SliderValue
	out :=  fullVarName " := """ Gdip_EncodeBitmapTo64string( Main.OutPBitmap , "PNG" , 100 ) """"
	startpos := 1
	...
	Clipboard := substr( output , 1 , StrLen( output ) - 1 ) "`n`n" Main.OutputVarName " := B64ToHBitmap(" fullVarName ")`n`n" ClipBitmapFunction()
...
AdjustSlider:
	...
	if (out > Main.SliderValue)
	{
		Main.Position.H := Main.Position.W := (out >= 300 ? 300 : out)
	...
	else if (out < Main.SliderValue)
	{
		Main.Position.H := Main.Position.W := (out < 10 ? 10 : out)
		Main.Gui1.ClearWindow()
		Main.Gui1.DrawBitmap( HB_BITMAP_MAKER( Main.Position ) , { X: 0 , Y: 0 , W: Main.Gui1.W , H: Main.Gui1.H } , dispose := 1 , AutoUpdate := 1 )
	}
...
CaptureScreen:
	...
	While( !GetKeyState( "ctrl" ) ){
		ToolTip, % "Press ""Ctrl"" to capture icon. `n"
		         . "Shift + MouseWheel to adjust size. Alt + MouseWheel for fine adjustments.`n"
		         . "Use Arrow keys to adjust position"
		         , x + 30, y + 30
		        ;  , x + Main.Position.W, y + Main.Position.H
...
DrawRing( obj , G ){
	...
	GuiControlGet, roundIcon, % obj.GuiHwnd ":" , %  obj.RoundCheckBoxHwnd
	if( check ){
...
CreateCroppedIconBitmap( main ){ ; https://www.autohotkey.com/boards/viewtopic.php?f=76&t=101960&p=453285#p453292
	GuiControlGet, roundIcon, % main.GuiHwnd ":" , %  main.RoundCheckBoxHwnd
	...
	if (roundIcon)
		Gdip_FillEllipse( Main.G1 , Brush , 1 , 1 , Main.Position.W - 2 , Main.Position.H - 2 )
	else
		Gdip_FillRectangle( Main.G1 , Brush , 1 , 1 , Main.Position.W - 2 , Main.Position.H - 2 )
...
#If ( Main.Active )
	...
	!WheelUp::
		( ( Main.Position.W += 3 ) > 300 ) ? ( Main.Position.W := 300 ) : ( Main.Position.H := Main.Position.W , Main.Gui1.ClearWindow() , Main.Gui1.DrawBitmap( HB_BITMAP_MAKER( Main.Position ) , { X: 0 , Y: 0 , W: Main.Gui1.W , H: Main.Gui1.H } , dispose := 1 , AutoUpdate := 1 ) )
		sleep, 30
		return
	!WheelDown::
		( ( Main.Position.W -= 3 ) < 10 ) ? ( Main.Position.W := 10 ) : ( Main.Position.H := Main.Position.W , Main.Gui1.ClearWindow() , Main.Gui1.DrawBitmap( HB_BITMAP_MAKER( Main.Position ) , { X: 0 , Y: 0 , W: Main.Gui1.W , H: Main.Gui1.H } , dispose := 1 , AutoUpdate := 1 ) )
		sleep, 30
		return
	+WheelUp::
		( ( Main.Position.W += 15 ) > 300 ) ? ( Main.Position.W := 300 ) : ( Main.Position.H := Main.Position.W , Main.Gui1.ClearWindow() , Main.Gui1.DrawBitmap( HB_BITMAP_MAKER( Main.Position ) , { X: 0 , Y: 0 , W: Main.Gui1.W , H: Main.Gui1.H } , dispose := 1 , AutoUpdate := 1 ) )
		sleep, 30
		return
	+WheelDown::
		( ( Main.Position.W -= 15 ) < 10 ) ? ( Main.Position.W := 10 ) : ( Main.Position.H := Main.Position.W , Main.Gui1.ClearWindow() , Main.Gui1.DrawBitmap( HB_BITMAP_MAKER( Main.Position ) , { X: 0 , Y: 0 , W: Main.Gui1.W , H: Main.Gui1.H } , dispose := 1 , AutoUpdate := 1 ) )
		sleep, 30
		return
...
ClipBitmapFunction(){
	...
	(Ltrim
		B64ToHBitmap(Input)
		{
			`tif !DllCall("GetModuleHandle", "str", "gdiplus", "Ptr")
				`t`tDllCall("LoadLibrary", "str", "gdiplus")
			
			`tVarSetCapacity(si, A_PtrSize = 8 ? 24 : 16, 0), si := Chr(1)
			`tDllCall("gdiplus\GdiplusStartup", "UPtr*", pToken:=0, "Ptr", &si, "Ptr", 0)

			`tVarSetCapacity(B64,strlen(Input) << !!A_IsUnicode)
			`tB64 := Input
			`tIf !DllCall("Crypt32.dll\CryptStringToBinary", "Ptr",&B64, "UInt", 0, "UInt", 0x01 , "Ptr", 0, "UIntP", DecLen, "Ptr", 0, "Ptr", 0)
				`t`tReturn False
			`tVarSetCapacity(Dec,DecLen,0)
			`tIf !DllCall("Crypt32.dll\CryptStringToBinary", "Ptr", &B64, "UInt", 0, "UInt", 0x01, "Ptr", &Dec, "UIntP", DecLen, "Ptr", 0, "Ptr", 0)
				`t`tReturn False
			
			`thData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2,  "UPtr",DecLen, "UPtr")
			`tpData := DllCall("Kernel32.dll\GlobalLock", "Ptr", hData, "UPtr")
			
			`tDllCall("Kernel32.dll\RtlMoveMemory", "Ptr", pData,"Ptr", &Dec, "UPtr", DecLen)
			`tDllCall("Kernel32.dll\GlobalUnlock", "Ptr", hData)
			`tDllCall("Ole32.dll\CreateStreamOnHGlobal", "Ptr", hData, "Int", True, "UPtr*", pStream)
			`tDllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  "Ptr", pStream, "UPtr*", pBitmap)
			`tDllCall("gdiplus\GdipCreateHBITMAPFromBitmap", "UPtr", pBitmap, "UPtr*", HBITMAP, "Int", Background := 0xffffffff)
