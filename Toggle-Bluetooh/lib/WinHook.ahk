...
class WinHook
{
	class Shell
	{
		...
		{
			if !WinHook.Shell.HasProp('Hooks')
			{
		...
		static Remove(Index)
		{
			...
			WinHook.Shell.Events[Hook.Event] := {}	; delete and rebuild Event list
		...
		static Message(Event, hwnd, msg, hwnd1)  ; Private Method
		{
	...
	class Event
	{
		...
		{
			if !WinHook.Event.HasProp('Hooks')
			{
