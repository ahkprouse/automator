...
;*******************************************************
	...
	Version:	1.01
	Link:		www.autohotkey.com/community/viewtopic.php?f=2&t=89246
	...
	2) link to BarChart's AutoHotkey forum post. 
	Author is not responsible for any damages arising from use or redistribution of his work. 
	If redistributed in source form, you are not allowed to remove comments from this file. 
		If you are interested in commercial usage, contact the author to get permission for it.
	...
	=ChartData=
	A string that contains chart data. It can be a table with two columns like;
	Mike	24
	Jenny	22
	Steve	27
	Marry	23
or a table with just one column (a list) which contains only numbers, like;
	...
	=Width, Height=
	[Case A] If Width is a number, than Width and Height parameters represent dimensions of a bitmap in which BarChart will be drawn. BarChart() will return a pointer to BarChart bitmap (pBitmap). Use this bitmap to set it to picture control in your GUI, create layered window from it, save it to file, set it to clipboard, etc. To create and use bitmap, call Gdip_Startup() first, than BarChart(), than do what you want with bitmap, than Gdip_DisposeImage(), and finally Gdip_Shutdown().
	[Case B] If Width is not a number, than it can be : 1) name of the picture control's associated variable or 2) a string "hwnd" followed by the variable which holds a handle of the picture control. In this case, BarChart() will automaticaly get picture control's width and height, create Barchart bitmap whose width and height will be equal to picture control's width and height, set bitmap to picture control and than dispose of that bitmap. BarChart()'s return value will be blank (it will not return pBitmap). In this case, Height parameter represents Gui number. If blank, it is set to 1.
	
	BarChart is centered in its width and height. BarChart is cropped (not scaled) if it is bigger than specified bitmap's width and height.
	
	=TitleText=
	Optional. BarChart's title. If blank, entire title bar in chart won't be drawn.
	
	=Skin=
	Optional. It is a set of BarChart's pre-defined graphical attributes. You can create your own skins, and override pre-defined skin attributes. Here is a list of pre-defines skins;
	...
	=Options=
	Optional. Here you can set and override a bunch of BarChart's pre-defined graphical and other attributes like; BarSpacing, BarRoundness, BarHeightFactor, MaxValue, TextSize, TextFont, DataAlignment, TitleAlignment, ValuesSuffix, BackgroundImage and many more. For a full list you'll have to look in the code. Look at examples to learn how to use them.
	...
	If GDI+ was not initialized and Width parameter is not a number, than BarChart() will automatically call Gdip_Startup(), create BarChart bitmap, put it to picture control, dispose of bitmap and call Gdip_Shutdown(). In other cases, user has to call Gdip_Startup() and Gdip_Shutdown() functions himself.
		
	=Good & bad GDI+ management when setting BarChart directly to picture control, without returning pBitmap=
	---------------------------------------------------------
	pToken := Gdip_Startup()
	BarChart(ChartData1, "Pic1")	
	BarChart(ChartData2, "Pic2")	
	BarChart(ChartData3, "Pic3")
	Gdip_Shutdown(pToken)
; this is GOOD - in this case GDI+ will be started up and shut down only once, by the user. Each BarChart() will recognize that GDI+ is already started up and it will not start up GDI+ again and will not shut it down.
	
	---------------------------------------------------------
	BarChart(ChartData1, "Pic1")	
	BarChart(ChartData2, "Pic2")	
	BarChart(ChartData3, "Pic3")
; this is BAD - in this case GDI+ will be started up and shut down 3 times. Due to fact that GDI+ was not started up in the moment when user calls BarChart(), every BarChart() call will internally start up and shut down GDI+.
	
	---------------------------------------------------------
	BarChart(ChartData1, "Pic1")
;  this is GOOD - in this case, where we have just one BarChart() call and no other actions which need GDI+ started up, GDI+ will be started up and shut down only once, by the BarChart() internally.
	
	---------------------------------------------------------
	pToken := Gdip_Startup()
	BarChart(ChartData1, "Pic1")	
	Gdip_Shutdown(pToken)
; this is GOOD - in this case GDI+ will be started up and shut down only once, by the user.
*/
...
BarChart(ChartData, Width, Height="", TitleText="", Skin="LightOrange", Options="") {	; by Learning one
	...
	if Width is not number
	{
		...
		if (SubStr(Width,1,4) = "hwnd") {	; extra feature for AHK_L users - use  handle of the control
	...
	{
		if SetBitmap2Control	; do the job for the user
			pToken := Gdip_Startup(), pBitmap := Gdip_CreateBitmap(Width, Height)
		else	; don't allow this case
			return
	}
	...
	if BackgroundImage {
		if (SubStr(BackgroundImage,1,3) = "ptr") {	; if user passes pointer to bitmap. Example: "ptr" pBackgroundBitmap
	...
	if SetBitmap2Control
	{
		...
		if pToken	; if this function started up GDI+, than shut it down
			Gdip_Shutdown(pToken)
	}
	else	
		return pBitmap
