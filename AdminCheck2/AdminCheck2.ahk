...
;@Ahk2Exe-SetProductName AdminCheck
;@Ahk2Exe-SetDescription This tool shows the programs that are run as admin
/**
 * ============================================================================ *
 * @Author   : Isaias Baez                                                      *
 * @Homepage :                                                                  *
 ...
 * ============================================================================ *
 * License:                                                                     *
 * Copyright Â©2022 The Automator <GPLv3>                                        *
 *                                                                              *
...
global script := {base         : script
                 ...
                 ,version      : "0.3.0"
                 ,author       : "Isaias Baez"
                 ,email        : "ibaez@the-automator.com"
                 ,crtdate      : "August 09, 2022"
                 ,moddate      : "August 12, 2022"
                 ,homepagetext : ""
                 ,homepagelink : ""
                 ,donateLink   : "https://www.paypal.com/donate?hosted_button_id=MBT5HSD9G94N6"}

try script.update("https://raw.githubusercontent.com/RaptorX/AdminCheck/devel/ver"
                 ,"https://github.com/RaptorX/AdminCheck/releases/download/latest/AdminCheck.zip")
...
Menu, Help, Add, Get Force Admin Code, menuHandler
Menu, Help, Add
Menu, Help, Add, How to use, menuHandler
Menu, Help, Add, How to run as admin, menuHandler
Menu, Main, Add, Help, :Help

Gui, pList:new, +AlwaysOnTop, % "Admin Process List"
Gui, font, s12
Gui, Menu, Main

Gui, add, ListView, w600 h700 Sort vmainLV, % "ID|Process Name|is Admin"
Gui Show
...
menuHandler(ItemName, ItemPos, MenuName) {
	...
	case 1:
		...
		(Ltrim
			...
			; If the script needs to be run at an elevated level, this takes care of that
			; https://www.autohotkey.com/docs/Variables.htm#IsAdmin
			if !A_IsAdmin
			{
				`tRun, *Runas %A_ScriptFullPath%
				`tExitApp
			}"
		)

		ToolTip, % "AHK code copied to clipboard"
		Sleep, 1500
		ToolTip
	case 3:
		Run https://www.the-automator.com/AdminCheckHowTo?src=prg
	case 4:
		Run https://www.the-automator.com/AdminCheckRunAs?src=prg
	}
}
...
updateList() {
	...
	Loop, % processList
	{
		hwnd := processList%A_Index%
		WinGet, pid, PID, ahk_id %hwnd%
		WinGet, pPath, processPath, ahk_pid %pid%
		WinGet, pName, processName, ahk_pid %pid%
		
		if !IsProcessElevated(pid)
			Continue
		
		pList[pid] := [pid,pName,IsProcessElevated(pid) ? "yes" : ""]

	}
	
	if pList.Count() == oldSize
		return
	...
	for each,process in pList
	{
		...
		ToolTip, % "Last added: " pName
		
		row := LV_Add("", process*)

		LV_Modify(row, "Select")
		if !WinActive("Admin Process List")
			WinActivate, % "Admin Process List"
	}

	if firstTime
		LV_Modify(0, "-Select")

	LV_ModifyCol(), LV_ModifyCol(3, "AutoHDR")
